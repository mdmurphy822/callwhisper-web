...
class HealthChecker:
    ...
    def check_disk_space(self, path: Optional[Path] = None) -> CheckResult:
        ...
        try:
            ...
            return CheckResult(
                name="disk_space",
                healthy=healthy,
                message=f"{free_gb:.1f} GB free" if healthy else f"Low disk space: {free_gb:.1f} GB",
                details={
    ...
    def check_memory(self) -> CheckResult:
        ...
        try:
            ...
            return CheckResult(
                name="memory",
                healthy=healthy,
                message=f"{available_mb:.0f} MB available" if healthy else f"Low memory: {available_mb:.0f} MB",
                details={
    ...
    def check_audio_device(self, device_name: Optional[str] = None) -> CheckResult:
        ...
        try:
            ...
            tool = "pactl" if backend == "pulse" else "arecord" if backend == "alsa" else "ffmpeg"
            return CheckResult(
                name="audio_device",
                healthy=False,
    ...
    def check_recordings_dir(self, recordings_dir: Optional[Path] = None) -> CheckResult:
    ...
    async def run_all_checks(
        ...
        healthy = all(
            c.healthy or (c.details and c.details.get("skipped"))
            for c in checks
        )
